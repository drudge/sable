...
func Format(ctx context.Context) error {
	...
	return run(ctx, nil, "gofmt", "-w", "magefile.go", "magefile_test.go")
}
...
// Verify checks generated source, Mage helpers, application tests, and static analysis.
func Verify(ctx context.Context) error {
	if err := CheckGenerated(ctx); err != nil {
		return err
	}
	if err := run(ctx, nil, "go", "test", "-tags=mage", "."); err != nil {
		return err
	}
...
// ReleaseGate exercises the exact release-critical paths before CI creates a tag.
func ReleaseGate(ctx context.Context) error {
	if err := Verify(ctx); err != nil {
		return err
	}
	if err := ReleaseSmoke(ctx); err != nil {
	...
	return DockerSmoke(ctx)
}

// ValidateReleaseVersion checks a release-workflow input without changing Git.
func ValidateReleaseVersion(version string) error {
	_, err := parseReleaseVersion(version)
	return err
}

// Publish uploads the current annotated tag through GoReleaser. Release
// orchestration belongs to GitHub Actions; the guard prevents accidental local
// publication while leaving the workflow retryable.
func Publish(ctx context.Context) error {
	if os.Getenv(githubActionsEnvironment) != "true" {
		return errors.New("publishing is restricted to the GitHub Actions release workflow")
	}
	if err := requireCleanWorktree(ctx); err != nil {
		return err
	}
	tag := strings.TrimSpace(os.Getenv(releaseTagEnvironment))
	if err := requireCurrentReleaseTag(ctx, tag); err != nil {
		return err
	}
	if err := requireDocker(ctx); err != nil {
		return err
...
func parseReleaseVersion(version string) (string, error) {
	if version != strings.TrimSpace(version) {
		return "", fmt.Errorf("release version %q contains leading or trailing whitespace", version)
	}
	...
	if len(release) > 128 {
		return "", fmt.Errorf("release version %q is too long for a container tag", version)
	}
	return release, nil
}
...
func requireCurrentReleaseTag(ctx context.Context, tag string) error {
	if tag == "" {
		return fmt.Errorf("%s must name the release tag", releaseTagEnvironment)
	}
	if _, err := parseReleaseVersion(tag); err != nil {
		return err
	}
	tagCommit, err := output(ctx, "git", "rev-list", "-n", "1", "refs/tags/"+tag)
	if err != nil {
		return fmt.Errorf("resolve release tag %s: %w", tag, err)
	}
	headCommit, err := output(ctx, "git", "rev-parse", "HEAD")
	if err != nil {
		return fmt.Errorf("resolve release commit: %w", err)
	}
	if tagCommit != headCommit {
		return fmt.Errorf("release tag %s points to %s, not current commit %s", tag, tagCommit, headCommit)
	}
	kind, err := output(ctx, "git", "cat-file", "-t", "refs/tags/"+tag)
	if err != nil {
