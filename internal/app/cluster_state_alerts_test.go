...
// alertNode is one node's alert state: its configuration, its vault, and its
// browsers.
...
func newAlertNode(t *testing.T, configuration config.Config, secrets map[string]alerts.Secrets, subscriptions ...webpush.Subscription) *alertNode {
	t.Helper()
	ctx := context.Background()
	node := &alertNode{path: filepath.Join(t.TempDir(), "sable.toml"), vault: &memoryBackupVault{}}
...
func replicate(t *testing.T, primary, replica *alertNode) {
	t.Helper()
...
func (node *alertNode) alertSettings(t *testing.T) []byte {
	t.Helper()
...
func TestClusterStateReplicatesAlertsWithTheirSecretsAndBrowsers(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	primary := newAlertNode(t, primaryAlertConfiguration(), primaryAlertSecrets(), laptop, phone)
	...
	replica := newAlertNode(t, replicaConfiguration, map[string]alerts.Secrets{
...
func TestClusterStateAlertReplicationSettlesAndFollowsChanges(t *testing.T) {
	...
	for _, test := range []struct {
		...
		change func(t *testing.T, primary *alertNode)
		...
		browserWrites int
		check         func(t *testing.T, replica *alertNode)
	}{
		{name: "nothing changed", change: func(*testing.T, *alertNode) {}},
		{
		...
		t.Run(test.name, func(t *testing.T) {
			t.Parallel()
			primary := newAlertNode(t, primaryAlertConfiguration(), primaryAlertSecrets(), laptop, phone)
			replica := newAlertNode(t, config.Defaults(), nil)
			replicate(t, primary, replica)
...
func TestClusterStateFromAnOlderPrimaryLeavesAlertsAlone(t *testing.T) {
	t.Parallel()
	ctx := context.Background()
	older := newAlertNode(t, config.Defaults(), nil)
	older.replicator.setAlerts(nil, nil, nil)
	replica := newAlertNode(t, primaryAlertConfiguration(), primaryAlertSecrets(), laptop)
